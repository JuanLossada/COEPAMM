...
export default function History() {
  return (
    <section
      id="historia"
      className="border-y border-navy-ink/[0.08] bg-mist px-4 py-16 sm:px-6 sm:py-28"
    >
      <div className="mx-auto max-w-6xl">
        ...
        <ol className="mt-10 flex flex-col sm:mt-16">
          {historia.map((h, i) => (
            <li
              key={h.anio}
              data-reveal=""
              data-reveal-delay={String(i * 80)}
              className="grid gap-3 border-t border-navy-ink/10 py-6 sm:grid-cols-[160px_1fr] sm:gap-12 sm:py-8.5"
            >
              <p className="font-display text-[32px] font-semibold leading-none text-navy-deep tabular-nums sm:text-[40px]">
                {h.anio}
              </p>
              <div>
